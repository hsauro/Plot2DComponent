...
begin
  ...
  begin
    ...
    begin
      // Only overwrite when the value is a string we actually recognise;
      // unknown strings (or non-string values) keep the current default.
      if Pair.JsonValue is TJSONString then
        if ParseLineStyle(Pair.JsonValue.Value, LS) then
...
begin
  ...
  try
    ...
    Exit;   // I/O erNow its time for a new features. ror � leave defaults alone
  end;
